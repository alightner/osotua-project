...
# mutual defection script (faked need among isotuatin)
...
for(v in 1:num_simulations){
  ...
  for(t in 1:duration){
    ...
    for(z in defect_node){
      if(!(V(g)[defect_node[z]] %in% nlist)){
        if(runif(1,0,1) <= dfpr){
          nlist <- c(nlist,V(g)[defect_node[z]])
          V(g)[defect_node[z]]$req <- sample(rlist,1)      # mean and sd?
        }
